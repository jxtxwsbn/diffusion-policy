    ...
    def forward(self, x):
        ...
        if self.dims == 3:
            if len(x.shape) == 6:
                ...
                N, C, D1, D2, H, W = x.shape
                x_5d = x.view(N * C * D1, D2, H, W)

                # Interpolate
                x_5d = F.interpolate(x_5d, size=(H * 2, W * 2), mode='nearest')

                # Reshape back to 6D tensor
                x = x_5d.view(N, C, D1, D2, H * 2, W * 2)
            else:
                x = F.interpolate(
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        if x.ndim == 6:
            ...
            x = rearrange(x, "b c f x y z -> (b f) c (x y z)")
            qkv = self.qkv(self.norm(x))
            h = self.attention(qkv)
            h = self.proj_out(h)
            res = rearrange((x + h), "(b f) c (x y z) -> b c f x y z", c=c, f=f, x=spatial[0], y=spatial[1], z=spatial[2])
        else:
            # print(x.shape,'===')
            # x = x.unsqueeze(dim=2)
            b, c, f, *spatial = x.shape
            ...
            h = self.attention(qkv)
            h = self.proj_out(h)
            res = rearrange((x + h), "(b f) c (x y) -> b c f x y", c=c, f=f, x=spatial[0], y=spatial[1])
            # res = res.squeeze(dim=2)
            # print(res.shape)
        return res
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        :return: an [N x C x ...] Tensor of outputs.
        """
        # print(self.num_classes,y,self.task_tokens)
