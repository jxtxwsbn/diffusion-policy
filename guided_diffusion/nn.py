...
class Conv3d(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # self.threed_spatial_conv = nn.Conv3d(dim, dim_out, kernel_size = kernel_size, padding = kernel_size // 2, stride = stride)
    ...
    def forward(
        ...
        if x.ndim == 6:
            x = rearrange(x, 'b c f h w d -> (b f) c h w d')
            # x = self.threed_spatial_conv(x)
            x = rearrange(x, '(b f) c h w d -> b c f h w d', b = b)

            if ignore_time or not exists(self.temporal_conv):
                return x

            h, w, d = x.shape[-3:]
            x = rearrange(x, 'b c f h w d -> (b h w d) c f')

            if self.kernel_size > 1:
                x = F.pad(x, (self.kernel_size//2, self.kernel_size//2))

            x = self.temporal_conv(x)
            x = rearrange(x, '(b h w d) c f -> b c f h w d', h = h, w = w, d = d)
        else:    
